...
function scene_setup()
{
    ...
    renderer = new THREE.WebGLRenderer( { canvas: canvas, antialias: true } ); //antialias for nitidez
...
function create_cube()
{
    ...
    let material = new THREE.MeshBasicMaterial({ map: texture }); // like mapping the vertexes in WebGL pure

    // Create the cube geometry          // x,y,z
    let geometry = new THREE.CubeGeometry(2, 2, 2);

    // And put the geometry and material together into a mesh
    cube = new THREE.Mesh(geometry, material); // mesh is material with geometry
    ...
    for ( let i = 0; i < geometry.faces.length; i +=2 ) 
    {
        ...
        // se pine dos veces, porque se asignan los colores a lso tríangulos! son dos por cara
